...
class Level2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        gameTimeLeft.text = "Time Left : 40s"
        self.lose = false
        self.shuffleTiles()
    ...
    override func viewDidAppear(_ animated: Bool) {
        let alert = UIAlertController(title: "开始第二关", message: "翻转并消除16张卡牌，你有40秒的时间！", preferredStyle: .alert)
    ...
    func winner() {
        timer.invalidate()
        timerEnable = false
        self.gameTimeLeft.text = "You passed level 2!"
        isEnablelevel3 = true
        let alert = UIAlertController(title: "游戏胜利", message: "level 3 已解锁", preferredStyle: .alert)
        let okAction = UIAlertAction(title: "确定", style: .default, handler: nil)
        alert.addAction(okAction)
        self.present(alert, animated: true, completion: nil)
    }
